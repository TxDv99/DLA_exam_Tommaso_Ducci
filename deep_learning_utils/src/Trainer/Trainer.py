...
class Trainer:
    ...
    def train(self, data_split, batch_size, num_epochs,iter_time = None, early_stopping = 5, val_check = 3, checks = True, use_wandb = True, project_name = None, augmentation = None, fgsm = None):
        ...
        if augmentation != None:
            ...
            transformed_train.dataset.transform = aug_transform
        ...
        for epoch in range(num_epochs):
            ...
            for (xs, ys) in tqdm(train_loader, desc=f'Training epoch {epoch}', leave=True):
                ...
                if fgsm is not None:
                    ...
                    budget = fgsm[0]
                    y_target = fgsm[1] if len(fgsm) == 2 else None
                    xs = _FGSM_batch(xs, ys, model=self.model, budget=budget, y_target=y_target, loss_fun=self.loss_fn)
...
def _FGSM_batch(x, y_true, model, budget=0.1, y_target=None, loss_fun=nn.CrossEntropyLoss()):
    model.eval()
    ...
    if y_target is not None:
        if isinstance(y_target, int):
            y_target = torch.full_like(y_true, y_target)
        assert y_target.shape == y_true.shape, "Target labels must match the shape of true labels"
        y = y_target
    else:
        y = y_true
